// src/balance.js 鈥?official DeepSeek account balance (C1, v0.2.4).
//
...
// and is NEVER logged, persisted to settings or shipped over IPC 鈥?only the
...
'use strict';
//
// ── 借用说明：本文件复制自 DshCockpit (https://github.com/Lxiayu/DshCockpit, MIT)，
//    仅追加此注释；逻辑保持原样以便跟随上游修复。
...
const BACKOFF_BASE_MS = 5 * 60_000;    // 1x -> 2x -> 4x of the poll interval鈥?
const BACKOFF_MAX_MS = 30 * 60_000;    // 鈥apped at 30 minutes
const LOW_BALANCE_FLOOR = 5;           // 楼 鈥?notify threshold floor
const LOW_BALANCE_BUDGET_PCT = 0.05;   // 鈥r 5% of the monthly budget
 ...
 * balance_infos may hold a CNY and/or a USD entry 鈥?CNY wins (mainland
 * accounts top up in CNY and the price matrix is in 楼). Returns null for
 * payloads without any usable entry (missing array, all-NaN numbers, 鈥?. */
...
function findApiKey(sources) {
  ...
  for (const file of [s.credentialsPath, s.envPath]) {
    ...
    // YAML `DEEPSEEK_API_KEY: sk-鈥 or dotenv `DEEPSEEK_API_KEY=sk-鈥
...
/** Backoff delay after the n-th consecutive failure: 1x, 2x, 4x 鈥?of the
...
/** Low-balance notify threshold: max(楼5, monthly budget x 5%). */
 ...
 * only 鈥?the key must never appear in messages or logs). */
 ...
 *   - readKey(): string   鈥?where the API key comes from (main wires env >
 *     credentials.yaml > .env); empty string = not configured
 *   - fetch(key): Promise<body> 鈥?transport (injectable for tests)
 *   - snapshotFile: string      鈥?persistence path
 *   - budgetOf(): number        鈥?monthly budget 楼 (drives the threshold)
 *   - onLowBalance(snap, threshold) 鈥?notification hook (fires once per
 *     descent below the threshold)
 *   - log(line)                 鈥?shell log (messages carry no key material)
 *   - now(): number             鈥?clock injection for tests
 * refresh({force}) is idempotent: one in-flight request at a time, 5-minute
 * throttle after a success, and consecutive failures back off 1x鈫?x鈫?x of
